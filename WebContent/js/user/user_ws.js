...
//	  		console.log(",를 기준으로 나눈 data : "+dataSplittedByComma);
	  		...
	  		$wrapChaircard.each(function () {
	  			
	  			var $this = $(this);
	  			
	  			var cafeNum = $this.data("no"); //해당 의자 카드의 카페 번호
	  			var chairIdx = $this.data("idx"); //해당 의자의 index 번호
	  			
	  			if(cafeNoFromIOTData == cafeNum) {
	  				if(currentSeatsArr[chairIdx-1] == 0) {
	  					$wrapChaircard.removeClass("on"); //div색깔 변화
	  					$wrapChaircard.find(".box_cafeStatus_icon>div>span").text("사용가능");
	  				} else {
	  					$wrapChaircard.addClass("on"); //div색깔 변화
	  					$wrapChaircard.find(".box_cafeStatus_icon>div>span").text("사용중");
	  				}//if~else end
	  			}//if end
...
var $wrapChaircard = $(".wrap_chair_card");
